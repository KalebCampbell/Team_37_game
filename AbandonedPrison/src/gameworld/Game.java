 ...
 * - Containers: WoodenBox MetalBox.
 * - Items: Crowbar Key Keycard.
 * - Rooms: Items in rooms Doors in rooms Room layout Room grid for object.
 * placement.
 * - Logic: Gameworld is responsible for handling.
 * 
...
public class Game {
  ...
  /**
   * Unlock container method.
   * @param container object to unlock.
   * @return true/false.
   */
  ...
  public boolean unlockDoor() {
    Room room = findRoom(player.getPlayerId());
    Door d = room.getDoorFacing(player.getDirection());

    if (d != null && d.isLocked()) {
      d.setLocked(false);
      System.out.println(d.getDirection() + " Door unlocked");
      return true;
    }
    return false;
  }
   ...
   * 
   * @param setup map file
   * @return Player initialised player
   */
   ...
   * 
   * @param setup game file
   * @return Inventory initialised inventory
   */
