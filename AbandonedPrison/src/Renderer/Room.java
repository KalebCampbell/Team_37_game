...
/**
 * Represents a Room in a 3D environment, with 0 - 4 walls, and a 2d array of Items within the Room.
 * 
 * @author Joel Harris
 */
public class Room implements Comparable<Room> {
   ...
   *          floor
   */
  public Room(Point3D position, ArrayList<AbstractWall> walls, AbstractItem[][] items,
      Floor floor) {
  ...
  /**
   * Uses the compareTo method in Item to order the Items within a Room using a PriorityQueue.
   * 
  ...
  public void addItem(AbstractItem item) {
    for (int i = 0; i < items.length; i++) {
      for (int j = 0; j < items.length; j++) {
        if (items[i][j] == null) {
          item.translate(-(Renderer.WHOLE_BLOCK + Renderer.HALF_BLOCK) + (Renderer.WHOLE_BLOCK * i),
              4, -(Renderer.WHOLE_BLOCK + Renderer.HALF_BLOCK) + (Renderer.WHOLE_BLOCK * i));
          items[i][j] = item;
          return;
        }
      }
   ...
   *          to be set
   */
  public void setRoom(GameWorld.Room value) {
    this.value = value;
  }
